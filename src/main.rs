...
#[embassy_executor::task]
async fn usb_midi_writer(mut usb_midi_tx: usb_midi::Sender<'static, usb::Driver<'static, USB>>) {
...
#[embassy_executor::task]
async fn usb_midi_reader(mut usb_midi_rx: usb_midi::Receiver<'static, usb::Driver<'static, USB>>) {
    // TODO: Handle the case where the USB cable gets connected/disconnected while the program is running.
...
async fn uart_writer(mut uart_tx: uart::UartTx<'static, uart::Async>) {
    loop {
        let _event = UART_EVENT_BUS.receive().await;
        let buf: [u8; 2] = [0u8, 0u8]; // event.serialize();
...
async fn main(spawner: Spawner) {
    ...
    let (_uart_tx, _uart_rx) = uart_txrx.split();
