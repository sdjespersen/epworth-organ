...
use epworth_organ::command::{Command, CommandType};
...
static COMMAND_BUS: Channel<CriticalSectionRawMutex, Command, 128> = Channel::new();
    ...
    loop {
        for (mcp_pair, div) in mcp_pairs.iter_mut().zip(DIVISIONS) {
            ...
            for i in stop_tab_button_debouncers[div as usize].falling_edges() {
                COMMAND_BUS
                    .send(Command::local(CommandType::StopToggle(div, i)))
                    .await;
            }
        }
...
async fn scan_pistons(
    ...
    loop {
        ...
        for i in piston_debouncer.falling_edges() {
            if i == 0 {
                COMMAND_BUS
                    .send(Command::local(CommandType::EnableSave(true)))
                    .await;
            } else if i == 1 {
                COMMAND_BUS
                    .send(Command::local(CommandType::GeneralCancel()))
                    .await;
            } else {
                COMMAND_BUS
                    .send(Command::local(CommandType::RecallPreset(i)))
                    .await;
            }
        }
        ...
        for i in piston_debouncer.rising_edges() {
            if i == 0 {
                COMMAND_BUS
                    .send(Command::local(CommandType::EnableSave(false)))
                    .await;
            }
        }
