...
use embassy_rp::{Peri, bind_interrupts, i2c, uart as rp_uart, usb as rp_usb};
...
static COMMAND_BUS: Channel<CriticalSectionRawMutex, (Command, CommandSource), 8> = Channel::new();

// Queues for events destined for the decoders (via UART) or the computer (via USB).
static UART_EVENT_BUS: Channel<CriticalSectionRawMutex, Event, 8> = Channel::new();
static USB_EVENT_BUS: Channel<CriticalSectionRawMutex, Event, 8> = Channel::new();
...
async fn usb_midi_reader(
    ...
    //             COMMAND_BUS.send((cmd, CommandSource::External)).await;
    //         }
    //         Err(_) => {
...
async fn uart_writer(mut uart_tx: rp_uart::BufferedUartTx) {
    loop {
        ...
        let mut buf = [0u8; 16];
        let written = to_slice(&event, &mut buf).unwrap();
