...
fn echo_event_to_usb(event: LiveEvent) {
    // This implementation looks very redundant, but it is so precisely because a LiveEvent may contain references to
    // data it does not own, so we need to create owned versions of the MIDI events to send over the channel.
    let event_out: Option<LiveEvent> = match event {
        LiveEvent::Midi { channel, message } => match message {
            MidiMessage::Controller { controller, value } => Some(LiveEvent::Midi {
                channel,
                message: MidiMessage::Controller { controller, value },
            }),
            MidiMessage::NoteOn { key, vel } => Some(LiveEvent::Midi {
                channel,
                message: MidiMessage::NoteOn { key, vel },
            }),
            MidiMessage::NoteOff { key, vel } => Some(LiveEvent::Midi {
                channel,
                message: MidiMessage::NoteOff { key, vel },
            }),
            MidiMessage::ProgramChange { program } => Some(LiveEvent::Midi {
                channel,
    ...
    match event_out {
        Some(e) => {
            let _ = OUTBOUND_USB_MIDI_EVENT_BUS.try_send(e);
        }
        _ => {}
    }
}
    ...
    loop {
        for (div, mcp_pair) in mcps.iter_mut().enumerate() {
            ...
            for i in stop_tab_button_debouncers[div].falling_edges() {
                EVENT_BUS
                    .send(Event::StopToggle {
                        src: EventSource::Indigenous,
                        div: div.into(),
                        idx: i,
                    })
                    .await;
...
async fn scan_pistons(
    ...
    loop {
        ...
        for i in piston_debouncer.falling_edges() {
            if i == 0 {
                // SAVE piston
                let enable_save_event = Event::EnableSave {
                    src: EventSource::Indigenous,
                    val: true,
                };
                EVENT_BUS.send(enable_save_event).await;
            } else if i == 1 {
                EVENT_BUS
                    .send(Event::GeneralCancel {
                        src: EventSource::Indigenous,
                    })
                    .await;
            } else {
                EVENT_BUS
                    .send(Event::RecallPreset {
                        src: EventSource::Indigenous,
                        idx: i,
                    })
                    .await;
...
#[embassy_executor::task]
async fn uart_midi_reader(mut uart_rx: uart::UartRx<'static, uart::Async>) {
    let mut stream = MidiStream::new();

    loop {
        // Should we use a 1-byte buffer instead?
        let mut buf: [u8; 3] = [0; 3];

        match uart_rx.read(&mut buf).await {
            Ok(_) => {
                stream.feed(&buf, echo_event_to_usb);
            }
            Err(e) => {
...
async fn main_event_handler(preset_store: &'static mut PresetStore<'static>) {
    ...
    loop {
        ...
        match event {
            Event::StopToggle { src, div, idx } => {
                let div_stop_state =
                    STOP_STATE[div as usize].fetch_xor(1 << idx, Ordering::SeqCst) ^ (1 << idx);
                STOP_STATE_CHANGED.signal(true);
                let msg = stop_tab_midi_cc_message(div.into(), idx, div_stop_state);
                let _ = OUTBOUND_USB_MIDI_EVENT_BUS.try_send(msg);
            }
            Event::GeneralCancel { src } => {
            ...
            Event::RecallPreset { src, idx } => {
                if awaiting_save {
                    // Saving a preset is completely internal; it emits no MIDI events.
                    save_preset(preset_store, idx - 1).await;
                } else {
                    ...
                    recall_preset(preset_store, idx - 1).await;
                }
            }
            Event::EnableSave { src, val } => {
                awaiting_save = val;
            }
            _ => {}
        }
...
async fn main(spawner: Spawner) {
    ...
    spawner.spawn(uart_midi_reader(uart_rx)).unwrap();
